...
object Async extends AsyncInterface {
  ...
  def sequenceComputations[A, B](
    ...
    makeAsyncComputation1().flatMap(Future.successful(_).zip(makeAsyncComputation2()))
  }
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] =
    if (maxAttempts <= 1)
      ...
      makeAsyncComputation().recoverWith(e => insist(makeAsyncComputation, maxAttempts - 1))
